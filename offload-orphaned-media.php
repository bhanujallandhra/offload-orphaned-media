...
class WP_Offload_Orphans_Unified {
    ...
    public function render_admin_page() {
        ...
        $nonce = wp_create_nonce( 'orphan_handler_nonce' );
        ?>
        <div class="wrap">
            <h1>Direct Upload Orphaned Files</h1>
            <p>Scan your upload directory for files not registered in the Media Library. Files successfully pushed to cloud storage are hidden from future scans.</p>
            ...
            jQuery(document).ready(function($) {
                ...
                let nonce = '<?php echo esc_js( $nonce ); ?>';
                ...
                $('#scan-orphans-btn').on('click', function() {
                    ...
                    $.post(ajaxurl, { 
                        action: 'scan_orphaned_files', 
                        blog_id: blogId, 
                        search: searchTerm, 
                        include_thumbs: includeThumbs,
                        _wpnonce: nonce
                    }, function(response) {
                        if(response.success) {
                            ...
                            html += '</ul>';
                            html += '<button id="bulk-process-btn" class="button button-primary button-large" data-blog="' + blogId + '">Direct Upload to Cloud Storage</button>';
                ...
                function processQueue(files, blogId, btn, index, successCount) {
                    if (index >= files.length) {
                        ...
                        btn.text('Direct Upload to Cloud Storage').removeAttr('disabled');
    ...
    public function ajax_clear_orphan_history() {
        check_ajax_referer( 'orphan_handler_nonce', '_wpnonce' );

        if ( is_multisite() && ! empty( $_POST['blog_id'] ) ) {
            switch_to_blog( absint( $_POST['blog_id'] ) );
        }
    ...
    /**
     * Optimizes PHP execution limits for large directory scans.
     * Only applies if user has proper admin capabilities.
     */
    private function optimize_execution_limits() {
        if ( ! current_user_can( 'manage_options' ) ) {
            return;
        }

        // Attempt to extend time limit for large scans
        if ( function_exists( 'set_time_limit' ) && false === strpos( ini_get( 'disable_functions' ), 'set_time_limit' ) ) {
            set_time_limit( 0 );
        }

        // Attempt to increase memory for large file lists
        if ( function_exists( 'ini_set' ) && false === strpos( ini_get( 'disable_functions' ), 'ini_set' ) ) {
            $current_limit = ini_get( 'memory_limit' );
            if ( $current_limit && intval( $current_limit ) < 1024 ) {
    ...
    public function ajax_scan_orphaned_files() {
        ...
        try {
            // Optimize execution for admin users only
            $this->optimize_execution_limits();
            ...
            foreach ( $iterator as $file ) {
                ...
                if ( preg_match( '/(\.htaccess|\.php|\.DS_Store|\.html)$/i', $filepath ) ) {
                    continue;
                }
                ...
                $cache_key = 'orphan_check_' . md5( $relative_path );
                $exists = wp_cache_get( $cache_key, 'orphan_handler' );
                
                if ( false === $exists ) {
                    // Direct query necessary - WordPress has no API for checking file attachment by path
                    ...
                    wp_cache_set( $cache_key, $exists, 'orphan_handler', 300 );
                }
    ...
    public function ajax_process_orphaned_file() {
        ...
        try {
            ...
            if ( $is_multisite ) {
                if ( empty( $_POST['blog_id'] ) ) {
                    wp_send_json_error( 'Missing site data.' );
                }
                $blog_id = absint( $_POST['blog_id'] );
                switch_to_blog( $blog_id );
            }

            $filepath = base64_decode( sanitize_text_field( wp_unslash( $_POST['file'] ) ) );
            ...
            if ( method_exists( $provider_wrapper, 'get_client' ) ) {
                try { 
                    $provider_wrapper->get_client( [ 'region' => $region ] ); 
                } catch ( \Throwable $e ) { 
                    try { 
                        $provider_wrapper->get_client(); 
                    } catch ( \Throwable $e2 ) {
            ...
            wp_send_json_success( 'Pushed directly to cloud storage.' );
